...
## 1. 簡介
對 MNIST 手寫數字辨識的全連接深度神經網路 (FC-DNN) 進行加速，最佳化硬體 Pipeline 結構並引入 Forwarding 技術，來降低運算時間與提升效能。修改後的程式碼主要在 `COLAB6b/practice` 中，測試答案正確後，複製 `COLAB6b/practice/im_data/im.txt` 的組語，取代 `COLAB6a/practice/hardware/im_data/im.txt`，在 FPGA 板上運行並記錄運算時間，實測以軟體和硬體方式最佳化前後的運算時間差距，並依此撰寫 5 頁的報告。
